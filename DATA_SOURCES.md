...
| **NOAA Space Weather Prediction Center** | Solar-wind speed, interplanetary magnetic field, planetary K index, and OVATION auroral grid | [NOAA data and information terms](https://www.noaa.gov/information-technology/open-data-dissemination) | "NOAA Space Weather Prediction Center" |
...
- **NOAA space weather.** The browser fetches four keyless SWPC JSON products directly: solar-wind speed, magnetic field, planetary K index, and the latest OVATION auroral grid. The full auroral grid is not stored; it is reduced in memory to one strongest point per five-degree longitude bucket and hemisphere before rendering.
